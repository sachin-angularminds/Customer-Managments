...
function Customeredit() {
    ...
    const onSubmit = async (data) => {
        ...
        const res = await fetch(`http://localhost:5000/custumers/${_id}`, {
            method: "put",
            body: formData
            }).then(res => res.json())
    ...
    const { firstName, lastName, occupation, dob, status, bio, profilePicture } = user;
    ...
    const edituser = async () => {
        await axios.get(`http://localhost:5000/custumers/${_id}`)
            .then(user => {
                const fields = ['firstName', 'lastName', 'occupation', 'status', 'bio', 'profilePicture'];
                fields.forEach(field => setValue(field, user.data[field]));
                setUser(user.data);
                setStartDate(user.data.dob);
                setFiles(user.data.file)
            });
    };
    return (
        <>
                ...
                <div className="card border-0 shadow p-4 w-50 mx-auto">
                    <form onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <div className="from-group" style={{ marginBottom: " 1rem" }}>
                            ...
                            <select  placeholder="Select Occupation" className={classNames("custom-select form-control", { "is-invalid": errors.occupation })} {...register('occupation', { required: "Occupation is required" })}>
                        ...
                        <div className="form-group">
                            ...
                            <DatePicker
                                ...
                                selected={startDate != null ? new Date(startDate) : new Date()}
                                onChange={(val) => {
                                    setStartDate(val);
                                    setValue("dob", val);
                                    setUser("dob", val)
                        ...
                        <div className="form-group" style={{ marginBottom: " 1rem" }}>
                            ...
                            <input type="file" data-testid="file" className={classNames({ "is-invalid": errors.file })} {...register('file', { required: false })} />
                            {errors.file &&
                        ...
                        <div className="form-group text-center">
                            <button  data-testid="update" type="submit" className="btn btn-primary">Update</button>
                        </div>
                    </form>
                </div>
            </div>
